/**
 * Popup script for Firefox Workspaces
 * Refactored to use DOM API instead of innerHTML
 */
...
/**
 * Build an emoji picker into a container
 */
...
function renderWorkspaces() {
    ...
    if (sorted.length === 0) {
        ...
        p2.className = 'hint';
        p2.textContent = 'Click "+ New" to create one';
    ...
    sorted.forEach(ws => {
        ...
        meta.textContent = `${ws.tabCount || 0} tabs • ${formatTime(ws.updatedAt)}`;
        if (isOpen) {
        ...
        const pinBtn = createBtn(ws.pinned ? '📍' : '📌', ws.pinned ? 'Unpin' : 'Pin', 'pin');
...
/**
 * Show/hide an inline emoji picker on a workspace card
 */
...
function startRename(workspaceId) {
    ...
    const finishRename = async (save) => {
        if (save && input.value.trim()) {
            ...
            await refreshWorkspaces(); // Revert by re-rendering
        }
    };
